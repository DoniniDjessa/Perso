...
export function MonthCalendar({
  ...
  const markedMap = new Map(marked.map((item) => [item.date, item.priority]))
      ...
      {Array.from({ length: cells.length / 7 }, (_, week) => (
        <XStack key={week} marginBottom={2}>
          {cells.slice(week * 7, week * 7 + 7).map((day, index) => {
            ...
            const eventPriority = markedMap.get(key)
            const ring = eventPriority ? priorityColor(eventPriority) : undefined
            return (
              <Pressable key={key} onPress={() => onSelect(day)} style={{ flex: 1 }}>
                <YStack height={44} alignItems="center" justifyContent="center">
                  <YStack
                    ...
                    backgroundColor={selectedDay ? colors.indigo : 'transparent'}
                  ...
                  >
                    <Text
                      style={{
                        ...fonts.semibold,
                        fontSize: 13,
                        color: selectedDay ? colors.white : today ? colors.indigo : colors.black,
                      }}
                    >
                      {day.getDate()}
                    </Text>
