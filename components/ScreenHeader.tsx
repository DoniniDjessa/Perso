...
import { useFormDrawer, type FormKind } from '@/components/FormDrawer'
...
function addKindFromPath(path: string): FormKind {
  if (path.includes('todos')) return 'todo'
  if (path.includes('agenda')) return 'agenda'
  if (path.includes('historique')) return 'history'
  if (path.includes('revenus')) return 'income'
  if (path.includes('credits')) return 'credit'
  return 'expense'
}
